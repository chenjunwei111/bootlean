...
layuiXtree.prototype.Initial = function (o) {
    ...
    _this._islinkage = o.islinkage != null ? o.islinkage : true;  //是否联动
    _this._ckallSuccess = o.ckallback != null ? o.ckallback : function () { };//全选回调
    _this.CreateCkAll();
    _this.dataBind(_this._dataJson);
    _this.Rendering();
    typeof o.dataBindEnd === 'function' && o.dataBindEnd(_this._dataJson);
    ...
    if (d!=null && d.length > 0) {
        for (i in d) {
            ...
            if (d[i].data != null && d[i].data.length > 0) {
                ...
                _this._domStr += '<i class="layui-icon layui-xtree-icon" '+xtree_ischecked+' data-xtree="' + (_this._isopen ? '1' : '0') + '">' + (_this._isopen ? _this._iconOpen : _this._iconClose) + '</i>';
            }
            else {
