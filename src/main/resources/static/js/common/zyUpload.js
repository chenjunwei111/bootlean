		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					...
					html += '		<div class="upload_main" style="min-height: 180px;">';
					...
					html += '	</div>';
					html += '</form>';
					html += '<div class="img_list" style="margin: 10px;">';
					html += '<h4 style="color: #85EFFF;font-size: 15px;padding: 10px;">已上传图片</h4>';
					html += '    <div class="img_upload">';
					html += '        <a class="add_imgBox" href="#" style="height: 100px;width: 120px;">';
					html += '          <div class="uploadImg">';
					html += '              <img class="upload_image" src="../../images/upload/add_img.png">';
					html += '              <img class="posit-ad larger" title="查看大图" src="../../images/upload/larger_image.png">';
					html += '              <img class="posit-ad delete" title="删除" src="../../images/upload/delete_image.png">';
					html += '          </div>';
					html += '        </a>';
					html += '    </div>';
					html += '</div>';
					...
					html += '<form id="uploadForm" action="'+para.url+'" method="post" enctype="multipart/form-data">';
					html += '	<div class="upload_box">';
					html += '		<div class="upload_main single_main" style="min-height: 180px;">';
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">选中0张文件，共0B。</div>';
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
		           // html += '					<div class="webuploader_pick">选择文件</div>';
		            ...
		            html += '					<div class="upload_btn2" id="fileConfirm">确认上传文件</div>';
                  //  html += '					<div class="upload_btn2" id="hideButton">图片确认</div>';
                    html += '				</div>';
		            html += '			</div>';
					...
					html += '<div class="img_list" style="margin: 10px;">';
					html += '<h4 style="color: #85EFFF;font-size: 15px;padding: 10px;">已上传图片</h4>';
					html += '    <div class="img_upload" id="photosDiv">';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if(file.type.indexOf("rar") > 0){
					...
					fileImgSrc = fileImgSrc + "static.js.image.file.png";
				}
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						...
						uploadPhoneCount ++;
						//alert("上传的照片数"+uploadPhoneCount +" ====== "+"所选的照片数"+selectPhoneCount);
						if(uploadPhoneCount == selectPhoneCount){
							successFlag = 1;
							setTimeout(function(){
								$(".saveWork").removeClass("layui-btn-disabled");
								$(".workSubmit").removeClass("layui-btn-disabled");
								$("#submitWorkTransfer").removeClass("layui-btn-disabled");
								$(".trunWork").removeClass("layui-btn-disabled");
								$("#submitFrozenWork").removeClass("layui-btn-disabled");
								if(clickTag == 'save'){
									$(".saveWork").click();
								}else if(clickTag == 'submit'){
									$(".workSubmit").click();
								}else if(clickTag == 'transfer'){
									submitWorkTransfer();
								}else if(clickTag == 'trunWork'){
									$(".trunWork").click();
								}else if(clickTag == 'frozenWork'){
									submitFrozenWork();
								}
					        },2000);
							$( "#vDistrictCITY_ADOPT_REASONS" ).trigger( "focus" );
							/*setTimeout(function(){
								//loadStop();
