...
layui.define(['element', 'common'], function(exports) {
	...
	Tab.prototype.tabAdd = function(data) {
        ...
        // console.log("_config:"+_config.elem)
        // for (var prop1 in _config.elem)
        // {
        //     console.log("属性 '" + prop1 + "' 为 " + _config.elem[prop1]);
        //
        // }
		if(tabIndex === -1) {
			//设置只能同时打开多少个tab选项卡
			if(_config.maxSetting !== 'undefined') {
                ...
                if(currentTabCount>=9){//只能打开7个
					msg("只能打开8个页面,请关掉一些页面");
					return;
                    // var $t=ELEM.titleBox.children('li').eq(1);
				...
				// if(typeof _config.maxSetting === 'number') {
                 //    console.log("2--maxSetting:"+_config.maxSetting );
				// 	if(currentTabCount === _config.maxSetting) {
                 //        console.log("3--currentTabCount:"+currentTabCount );
				// 		layer.msg('为了系统的流畅度，只能同时打开' + _config.maxSetting + '个选项卡。');
				// 		return;
				// 	}
				// }
				// if(typeof _config.maxSetting === 'object') {
				// 	console.log("3--object:"+_config.maxSetting);
		...
		if(_config.contextMenu) {
			...
			ELEM.titleBox.find('li').on('contextmenu', function(e) {
				...
				div.style.backgroundColor = '#15293F';
				div.style.border='1px solid #71c8b4';
				...
				ul += '<li data-target="closeOther" title="关闭其他选项卡"><i class="fa fa-close" aria-hidden="true"></i> 关闭其他</li>';
